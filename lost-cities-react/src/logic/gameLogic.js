...
function play(gameObj, playString) {
    ...
    if (inputArr.length !== 2) {
        ...
        throw new Error("Invalid input. Please enter 'play (handIndex)' or 'discard (handIndex)'");
      }
    else {
        let action = inputArr[0].toLowerCase();
        let handIndex = parseInt(inputArr[1]);
    
        if (action !== "play" && action !== "discard") {
            //throw new Error(colors.red("Invalid action. Please enter 'play (handIndex)' or 'discard (handIndex)'"));
            throw new Error("Invalid action. Please enter 'play (handIndex)' or 'discard (handIndex)'");
        }       
            ...
            throw new Error("Invalid hand index. Please enter a number from 0 to 7 for handIndex.");
        }
        else {
            ...
            if (action === 'play') {
                ...
                if (curVal) {
                    if (curVal.value > selectedCard.value) {
                        ...
                        throw new Error("Invalid Move: cannot place card of lower value");
                    }
                }
...
function draw(gameObj, drawString){
    ...
    if ((action !== "draw" && action !== "discard") || inputArr.length > 2) {
        ...
        throw new Error("Invalid input. Please enter 'draw' or 'discard (expedition)'.");
    }
                
    if (action === "draw") {
        if (inputArr.length > 1) {
            //throw new Error(colors.red("Invalid input. Please enter only 'draw' for drawing from the deck."));
            throw new Error("Invalid input. Please enter only 'draw' for drawing from the deck.");
        }
        else {
    ...
    if (action === "discard") {
        ...
        if (inputArr.length !== 2) {
            ...
            throw new Error("Invalid input. Please enter 'discard (expedition)'.");
        }           
            ...
            throw new Error("Invalid expedition. Please enter one of the following colors: red, green, white, blue, yellow.");
        }
        else {
            if (gameObj.discard[expedition].length > 0){
                ...
                throw new Error("Invalid Move: pile is empty");
            }
